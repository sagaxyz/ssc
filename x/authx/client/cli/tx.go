...
Examples:
 ...
 $ %s tx %s grant cosmos1skjw.. generic --msg-type=/cosmos.gov.v1.MsgVote --from=cosmos1sk..
	`, version.AppName, authx.ModuleName, version.AppName, authx.ModuleName),
		),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var authorization authx.Authorization
			switch args[1] {
			case "send":
			...
			case "generic":
				...
				authorization = authx.NewGenericAuthorization(msgType)
			...
			msg, err := authx.NewMsgGrant(clientCtx.GetFromAddress(), grantee, authorization, expire)
			if err != nil {
				return err
			}
...
Example:
 $ %s tx %s revoke cosmos1skj.. %s --from=cosmos1skj..
			`, version.AppName, authx.ModuleName, bank.SendAuthorization{}.MsgTypeURL()),
		),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			...
			msg := authx.NewMsgRevoke(granter, grantee, msgAuthorized)
...
Example:
 ...
 $ %s tx bank send <granter> <recipient> --from <granter> --chain-id <chain-id> --generate-only > tx.json && %s tx %s exec tx.json --from grantee
			`, version.AppName, authx.ModuleName, version.AppName, version.AppName, authx.ModuleName),
		),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := authx.NewMsgExec(grantee, theTx.GetMsgs())
