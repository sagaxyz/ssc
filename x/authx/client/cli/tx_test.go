...
import (
	...
	_ "cosmossdk.io/api/cosmos/authx/v1beta1"
	...
	authxclitestutil "github.com/cosmos/cosmos-sdk/x/authx/client/testutil"
...
func (s *CLITestSuite) SetupSuite() {
	...
	out, err := authxclitestutil.CreateGrant(s.clientCtx, []string{
		s.grantee[1].String(),
		"send",
	...
	_, err = authxclitestutil.CreateGrant(s.clientCtx, []string{
		s.grantee[2].String(),
		"send",
		fmt.Sprintf("--%s=100stake", cli.FlagSpendLimit),
		fmt.Sprintf("--%s=%s", flags.FlagFrom, val[0].Address.String()),
	...
	out, err = authxclitestutil.CreateGrant(s.clientCtx,
		[]string{
			s.grantee[3].String(),
			"send",
			fmt.Sprintf("--%s=100stake", cli.FlagSpendLimit),
...
func (s *CLITestSuite) TestCmdRevokeAuthorizations() {
	...
	// send-authorization
	_, err := authxclitestutil.CreateGrant(s.clientCtx,
		[]string{
			grantee.String(),
			"send",
			fmt.Sprintf("--%s=100stake", cli.FlagSpendLimit),
