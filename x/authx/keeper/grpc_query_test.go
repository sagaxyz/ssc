...
func (suite *TestSuite) TestGRPCQueryAuthorization() {
	...
	var (
		req              *authx.QueryGrantsRequest
		expAuthorization authx.Authorization
	)
	...
	testCases := []struct {
		...
		expError string
		postTest func(require *require.Assertions, res *authx.QueryGrantsResponse)
	}{
		{
			...
			func(require *require.Assertions, res *authx.QueryGrantsResponse) {},
		},
		{
...
func (suite *TestSuite) TestGRPCQueryGranterGrants() {
	...
	testCases := []struct {
		...
		preRun   func()
		expError bool
		request  authx.QueryGranterGrantsRequest
		numItems int
	}{
		{
			"fail invalid granter addr",
			func() {},
			true,
			authx.QueryGranterGrantsRequest{},
			0,
		},
		{
...
func (suite *TestSuite) TestGRPCQueryGranteeGrants() {
	...
	testCases := []struct {
		...
		preRun   func()
		expError bool
		request  authx.QueryGranteeGrantsRequest
		numItems int
	}{
		{
			"fail invalid granter addr",
			func() {},
			true,
			authx.QueryGranteeGrantsRequest{},
			0,
		},
		{
			"valid case, single authorization",
...
func (suite *TestSuite) createSendAuthorization(grantee, granter sdk.AccAddress) authx.Authorization {
	exp := suite.ctx.BlockHeader().Time.Add(time.Hour)
	newCoins := sdk.NewCoins(sdk.NewInt64Coin("steak", 100))
	authorization := &banktypes.SendAuthorization{SpendLimit: newCoins}
	err := suite.authxKeeper.SaveGrant(suite.ctx, grantee, granter, authorization, &exp)
	suite.Require().NoError(err)
	return authorization
}

func (suite *TestSuite) createSendAuthorizationWithAllowList(grantee, granter sdk.AccAddress) authx.Authorization {
	exp := suite.ctx.BlockHeader().Time.Add(time.Hour)
	newCoins := sdk.NewCoins(sdk.NewInt64Coin("steak", 100))
