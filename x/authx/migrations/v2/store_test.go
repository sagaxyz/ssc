...
import (
	...
	v2 "github.com/cosmos/cosmos-sdk/x/authx/migrations/v2"
...
func TestMigration(t *testing.T) {
	encodingConfig := moduletestutil.MakeTestEncodingConfig(authxmodule.AppModuleBasic{}, bank.AppModuleBasic{})
	cdc := encodingConfig.Codec

	authxKey := storetypes.NewKVStoreKey("authx")
	ctx := testutil.DefaultContext(authxKey, storetypes.NewTransientStoreKey("transient_test"))
	...
	grants := []struct {
		...
		msgType       string
		authorization func() authx.Grant
	}{
		{
			granter1,
			grantee1,
		...
		},
		{
			...
			genericMsgType,
			func() authx.Grant {
				cdcAny, err := codectypes.NewAnyWithValue(authx.NewGenericAuthorization(genericMsgType))
				require.NoError(t, err)
				return authx.Grant{
					Authorization: cdcAny,
					Expiration:    &oneYear,
	...
	storeService := runtime.NewKVStoreService(authxKey)
