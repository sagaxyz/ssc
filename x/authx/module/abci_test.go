...
func TestExpiredGrantsQueue(t *testing.T) {
	...
	accountKeeper := authxtestutil.NewMockAccountKeeper(ctrl)
	...
	authxKeeper := keeper.NewKeeper(storeService, encCfg.Codec, baseApp.MsgServiceRouter(), accountKeeper)

	save := func(grantee sdk.AccAddress, exp *time.Time) {
		err := authxKeeper.SaveGrant(ctx, grantee, granter, sendAuthz, exp)
	...
	authx.RegisterQueryServer(queryHelper, authxKeeper)
	queryClient := authx.NewQueryClient(queryHelper)

	checkGrants := func(ctx sdk.Context, expectedNum int) {
		require.NoError(t, authxmodule.BeginBlocker(ctx, authxKeeper))

		res, err := queryClient.GranterGrants(ctx.Context(), &authx.QueryGranterGrantsRequest{
