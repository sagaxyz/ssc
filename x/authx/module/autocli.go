...
import (
	...
	authxv1beta1 "cosmossdk.io/api/cosmos/authx/v1beta1"
...
func (am AppModule) AutoCLIOptions() *autocliv1.ModuleOptions {
	return &autocliv1.ModuleOptions{
		Query: &autocliv1.ServiceCommandDescriptor{
			Service: authxv1beta1.Query_ServiceDesc.ServiceName,
			RpcCommandOptions: []*autocliv1.RpcCommandOptions{
				{
					...
					Example:   fmt.Sprintf("%s query authx grants cosmos1skj.. cosmos1skjwj.. %s", version.AppName, bank.SendAuthorization{}.MsgTypeURL()),
		...
		Tx: &autocliv1.ServiceCommandDescriptor{
			Service:              authxv1beta1.Msg_ServiceDesc.ServiceName,
			...
			RpcCommandOptions: []*autocliv1.RpcCommandOptions{
				{
					...
					Example:   fmt.Sprintf("$ %s tx authx exec msg.json --from grantee\n $ %[1]s tx bank send [granter] [recipient] [amount] --generate-only | jq .body.messages > msg.json && %[1]s tx authx exec msg.json --from grantee", version.AppName),
					PositionalArgs: []*autocliv1.PositionalArgDescriptor{
				...
				},
				{
					...
					Example: fmt.Sprintf(`%s tx authx revoke cosmos1skj.. %s --from=cosmos1skj..`,
