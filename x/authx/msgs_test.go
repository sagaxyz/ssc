...
func TestMsgGrantGetAuthorization(t *testing.T) {
	...
	g := authx.GenericAuthorization{Msg: "some_type"}
	var err error
	...
	g = authx.GenericAuthorization{Msg: "some_type2"}
...
func TestAminoJSON(t *testing.T) {
	...
	authx.RegisterLegacyAminoCodec(legacyAmino)
	...
	grant, err := authx.NewGrant(blockTime, authx.NewGenericAuthorization(typeURL), &expiresAt)
	require.NoError(t, err)
	sendAuthz := banktypes.NewSendAuthorization(sdk.NewCoins(sdk.NewCoin("stake", sdkmath.NewInt(1000))), nil)
	sendGrant, err := authx.NewGrant(blockTime, sendAuthz, &expiresAt)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	delegateGrant, err := authx.NewGrant(blockTime, stakingAuth, nil)
	require.NoError(t, err)

	// Amino JSON encoding has changed in authx since v0.46.
	...
	tests := []struct {
		...
		{
			msg: &authx.MsgGrant{Granter: "cosmos1abc", Grantee: "cosmos1def", Grant: grant},
		...
		{
			msg: &authx.MsgGrant{Granter: "cosmos1abc", Grantee: "cosmos1def", Grant: sendGrant},
		...
		{
			msg: &authx.MsgGrant{Granter: "cosmos1abc", Grantee: "cosmos1def", Grant: delegateGrant},
		...
		{
			msg: &authx.MsgRevoke{Granter: "cosmos1abc", Grantee: "cosmos1def", MsgTypeUrl: typeURL},
		...
		{
			msg: &authx.MsgExec{Grantee: "cosmos1def", Msgs: []*cdctypes.Any{msgSendAny}},
