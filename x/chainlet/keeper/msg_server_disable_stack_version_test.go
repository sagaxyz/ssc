...
func (s *TestSuite) TestDisabledVersionsLaunch() {
	...
	_, err = s.msgServer.LaunchChainlet(s.ctx, types.NewMsgLaunchChainlet(
		creator.String(), nil, "test", ver, "test_chainlet", "test_12345-1", "asaga", types.ChainletParams{}, nil, false, "", nil,
	))
	s.Require().NoError(err)
	...
	_, err = s.msgServer.LaunchChainlet(s.ctx, types.NewMsgLaunchChainlet(
		creator.String(), nil, "test", ver, "test_chainlet", "test_12346-1", "asaga", types.ChainletParams{}, nil, false, "", nil,
	))
	s.Require().Error(err)
}
...
func (s *TestSuite) TestDisabledVersionAutoUpgrade() {
	...
	for i, tt := range tests {
		s.Run(fmt.Sprintf("%d: %s -> %s", i, tt.current, tt.expectedLatest), func() {
			...
			_, err = s.msgServer.LaunchChainlet(s.ctx, types.NewMsgLaunchChainlet(
				creator.String(), nil, "test", tt.current, "test_chainlet", chainId, "asaga", types.ChainletParams{}, nil, false, "", nil,
			))
			s.Require().NoError(err)
